...
def fetch(word, min_size=5e6):
    ...
    in_name = os.path.join(
        config.corpus_directory, config.template["index"].format(word)
    )
    ...
    if os.path.exists(out_name):
        try:
            ...
            with gzip.open(out_name, "rb") as f_in:
                ...
                print(
                    'Good file "{0}" below minimum size: {1} < {2}.'.format(
                        out_name, size, min_size
                    )
                )
...
def main():
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Perform a dry run only."
    )
