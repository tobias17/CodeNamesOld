...
def main():
    ...
    parser.add_argument(
        ...
        "present, or the last {0}.N in numerical order if not)".format(
            config.embedding
        ),
    )
    parser.add_argument(
        "--top-singles", type=int, default=10, help="Show top single matches."
    )
    parser.add_argument(
        "--top-pairs", type=int, default=0, help="Show top pair matches."
    )
    parser.add_argument(
        "--save-plots",
        type=str,
    ...
    if args.npass is not None:
        ...
        all_suffixes = [
            f.split(".")[-1] for f in glob.glob("{0}.*".format(config.embedding))
        ]
        evaluated_file = "{0}.{1}".format(config.embedding, sorted(all_suffixes)[-1])
    ...
    if args.top_singles > 0:
        ...
        for i in order[: args.top_singles]:
            ...
            print("{0:.3f} {1} = {2}".format(best_score[i], word.upper(), clue))
        if args.save_plots:
            plt.hist(best_score, range=(0.0, 1.0), bins=50)
            plt.xlim(0.0, 1.0)
    ...
    if args.top_pairs > 0:
        ...
        for i1, word1 in enumerate(words):
            for i2, word2 in enumerate(words[:i1]):
                clue, score = embedding.get_clue([word1, word2], [word1, word2], [], [])
                if clue:
                    best_score.append(score)
