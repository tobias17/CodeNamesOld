...
class ClueInfo:
    def __init__(self, clue_count):
        self.clue_dists = [[0 for i in range(HISTOGRAM_WIDTH)] for i in range(clue_count+1)]
...
class Tracker:
    ...
    def add(self, clue, words, e):
        ...
        self.clue_infos[index].neutral_dist_max[int(neutral_sims[0] * HISTOGRAM_WIDTH)] += 1
        for sim in neutral_sims:
            self.clue_infos[index].neutral_dist_avg[int(sim * HISTOGRAM_WIDTH)] += 1

        negative_sims = get_sims(clue, [str(word)[2:-1] for word in e.opponent_words], e)
        self.clue_infos[index].negative_dist_max[int(negative_sims[0] * HISTOGRAM_WIDTH)] += 1
    ...
    def save_to_file(self, filename):
        with open(filename, 'w+') as file:
            ...
            for i in range(9):
                ...
                for j in range(i+1):
                    file.write(','.join(str(word) for word in info.clue_dists[j]) + '\n')
                file.write(','.join(str(word) for word in info.neutral_dist_max) + '\n')
                file.write(','.join(str(word) for word in info.neutral_dist_avg) + '\n')
                file.write(','.join(str(word) for word in info.negative_dist_max) + '\n')
                file.write(','.join(str(word) for word in info.negative_dist_avg) + '\n')
    ...
    def load_from_file(self, filename):
        with open(filename, 'r') as file:
            ...
            for i in range(9):
                info = ClueInfo(i)
                for j in range(i+1):
                    info.clue_dists[j] = [int(item) for item in lines.pop().strip().split(',')]
                info.neutral_dist_max = [int(item) for item in lines.pop().strip().split(',')]
                info.neutral_dist_avg = [int(item) for item in lines.pop().strip().split(',')]
                info.negative_dist_max = [int(item) for item in lines.pop().strip().split(',')]
                info.negative_dist_avg = [int(item) for item in lines.pop().strip().split(',')]
                info.assassin_dist = [int(item) for item in lines.pop().strip().split(',')]
